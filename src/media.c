...
static int
media_load_texture(struct game_context *ctx, char *path, struct texture *t)
{
	int ret = 0;
...
static int media_load_cars_sprites_one_model(struct game_context *ctx,
					     ...
					     char *right2)
{
	media_load_texture(ctx, rear, &ctx->gfx.cars[idx][CAR_SPRITE_REAR]);
	media_load_texture(ctx, right1, &ctx->gfx.cars[idx][CAR_SPRITE_RIGHT1]);
	media_load_texture(ctx, right2, &ctx->gfx.cars[idx][CAR_SPRITE_RIGHT2]);
...
{
	media_load_cars_sprites_one_model(ctx,
					  CAR_MODEL_IMPREZIA,
					  PNG_IMPREZIA_REAR,
					  PNG_IMPREZIA_RIGHT1,
					  PNG_IMPREZIA_RIGHT2);
	...
	media_load_cars_sprites_one_model(
...
{
	media_load_texture(ctx, PNG_BG_MOUNTAINS, &ctx->gfx.bg_mountains);
	media_load_texture(ctx, PNG_BG_SKY_NEAR, &ctx->gfx.bg_sky_near);
	media_load_texture(ctx, PNG_BG_SKY_FAR, &ctx->gfx.bg_sky_far);
...
{
	media_load_texture(ctx, PNG_SMOKE_00, &ctx->gfx.t_smoke[0]);
	media_load_texture(ctx, PNG_SMOKE_01, &ctx->gfx.t_smoke[1]);
	media_load_texture(ctx, PNG_SMOKE_02, &ctx->gfx.t_smoke[2]);
	media_load_texture(ctx, PNG_SMOKE_03, &ctx->gfx.t_smoke[3]);
	media_load_texture(ctx, PNG_SMOKE_04, &ctx->gfx.t_smoke[4]);
	media_load_texture(ctx, PNG_SMOKE_05, &ctx->gfx.t_smoke[5]);
	media_load_texture(ctx, PNG_SMOKE_06, &ctx->gfx.t_smoke[6]);
	media_load_texture(ctx, PNG_NITRO_00, &ctx->gfx.t_nitro[0]);
	media_load_texture(ctx, PNG_NITRO_01, &ctx->gfx.t_nitro[1]);
	media_load_texture(ctx, PNG_NITRO_02, &ctx->gfx.t_nitro[2]);
	media_load_texture(ctx, PNG_NITRO_03, &ctx->gfx.t_nitro[3]);
	media_load_texture(ctx, PNG_NITRO_04, &ctx->gfx.t_nitro[4]);
